...
identity alias add bob {PublicKey} --context my_context
```

Everywhere in the tutorial, on the node which you have created the aliases (aliases are node specific),
you can use `my_context` and `bob` instead of the contextID and the invitorContextIdentity
:::
