---
id: building-with-etherium
title: Building with Etherium
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Building with Etherium

Step-by-Step Guide to Building an Application for Calimero with Etherium
...
- **Foundry**: For local Etherium development - 
...
Both contracts are written in Solidity, you can find their implementations in the
following repositories:

- [Context contract](https://github.com/calimero-network/calimero-evm-contracts/tree/master/context-config)
- [Proxy contract](https://github.com/calimero-network/calimero-evm-contracts/tree/master/context-proxy)

## Context and Proxy Contract

### Context contract

Context contract is deployed on Etherium sepolia network and implemented into Calimero.
...
```
[context.config.signer.self.ehterium.testnet]
rpc_url = <RPC_URL>
...
feature and are not real Etherium accounts. To create your own Etherium account
...
```bash title="Node1 Terminal"
context create <APPLICATION_ID> --protocol etherium
...
For more information about transferring funds checkout this [link](https://book.getfoundry.sh/reference/cast/cast-send)
To verify contract balance you can check it on Etherscan Explorer by visiting:
...
We have created a `<DEMO_EXTERNAL_CONTRACT_ID>` and deployed it on Etherium
sepolia.
...
- **setValueNoDeposit** - This method does not require a deposit for execution, it sets
...
- **Contract ID**: `{DEMO_EXTERNAL_CONTRACT_ID}` - Address of the external
  contract
- **Method Name**: `deposit(string, string)` - Name of the contract method that will be called with arguments types
...
Supported Etherium argument types:

- `bool` - Boolean value, representing true or false.
- `string` - Text string, used for storing sequences of characters.
- `address` - Contract or account address, a 20-byte value used to identify accounts.
...
- `uint256` - 256-bit unsigned integer, used for storing large non-negative integer values.
...
These types are used to define the parameters and return values of smart contract functions in Etherium-based blockchains.
...
application for Calimero and Etherium, from environment overview to deploying a
...
Etherium application for Calimero.
