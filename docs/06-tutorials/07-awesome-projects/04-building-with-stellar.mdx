...
Both contracts are written in Rust, you can find their implementations in the following repositories:
...
With each context creation a proxy contract for that context is deployed.
Proxy contract is used to handle blockchain operations such as cross-contract
calls and transfers as well as storing variables.
...
To be able to fund the contract we need to create a testnet account and seed it with tokens.
...
- `CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC` is the address of the
  Stellar testnet ledger contract.
...
- `100000000` is the amount of lumens you want to fund the contract with. (1 XLM = 10000000)
...
We have created a `<DEMO_EXTERNAL_CONTRACT_ID>` and deployed it on Stellar testnet.
...
- **no_deposit** - This method does not require a deposit for
  execution, it sets sent arguments in its storage.
- **deposit** - This method requires a deposit for execution, it first
...
- **Arguments**: The following arguments correspond to the deposit method parameters:
...
can be checked calling demo external contract method **get_value** with Stellar CLI:
...
The implementation of cross-contract call with deposit is based on the following steps:
1. The contract detects that an attached deposit is present.
2. It calls the ledger contract to enable the external contract (the
   contract being called) to withdraw funds.
...
solid understanding of the development workflow and how to build your own Stellar
