...
If you use `self` for signer then you also need to change `public` and `secret` key.
...
The values auto-generated only represent the format needed for self signing feature and are not 
real Stellar accounts. To create your own Stellar account with CLI follow the Stellar [Docs](https://developers.stellar.org/docs/build/guides/cli)


If signer is changed to `signer = "relayer"` you don't need to changed anything as
relayer will handle all the blockchain operations.
...
Which is caused by having multiple Stellar accounts and no default one is set.
To fix this re-run command replacing `--source default` with
