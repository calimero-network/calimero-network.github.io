...
To summarize, we're working on local-first, optimistic execution, which will ensure transactions are only executed once, and are appropriately signed which will prevent duplicate submissions.
