...
Admin dashboard is available at https://calimero-network.github.io/admin-dashboard/.

> **_NOTE:_**
> Update `NODE_PORT` in the placeholder `http://localhost:NODE_PORT` with the `--server-port` value defined during node setup in [Getting-started](./01-getting-started.mdx)
