...
```bash title="Terminal"
cargo run -p merod -- --node-name coordinator --home data init --server-port 2427 --swarm-port 2527
...
> **_NOTE:_** A `--protocol` flag is also available to specify your preferred
> protocol for the context. Currently, we support Starknet and NEAR protocol. By
> default, the protocol is set to NEAR protocol, but you can set it explicitly
> to Starknet by using the following command:
>
> ```bash title="Terminal"
> cargo run -p merod -- --node-name node1 --home data init --server-port 2428 --swarm-port 2528 --protocol starknet
> ```
>
> If you want to use NEAR protocol, you don’t need to specify the --protocol
> flag, as it’s the default protocol used by your node.
>
> You can also view these settings in the config.toml file located within the
> node folder in the storage directory.
>
> ![NearConfig](/init/near-config.png) >![SnConfgi](/init/sn-config.png)

### Docker Setup

You can also use Docker to setup the nodes. The outcome is the same as using
...
```bash title="Coordinator Terminal"
docker attach core-coordinator_run-1
```
