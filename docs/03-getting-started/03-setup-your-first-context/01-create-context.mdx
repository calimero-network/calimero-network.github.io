...
3. Choose one of the available options (Only Peers / Template App).
4. Press "Start" to initiate the context creation process with the selected
   application.

Visit [Admin Dashboard Page](../02-admin-dashboard.mdx) to view detailed instructions with images.
...
## Create context with the CLI using interactive CLI

After you starting the node following [instructions](../01-setup.mdx), interactive CLI will be available within the same terminal.

![InteractiveCLI](/cli/0-interactive-cli.png)

> **NOTE**: You can see available commands in terminal: call, peers, pool, gc, context, application.
> In this example we will be using commands for `context` and `application`.
...
You can find the application `URL` and `contractAppId` by opening
...
Conext can be created from the [Admin Dashboard Page](../02-admin-dashboard.mdx) or with interactive CLI.
