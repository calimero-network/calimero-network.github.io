...
1. If the node already has application installed, crating context using command:

`meroctl context create --application <app-id>`

2. If application is not installed then create context using command:
...
url you used while starting the node in [Getting-started](./01-setup.mdx)
`http://localhost:NODE_PORT` After setting up node url, you will be asked to
login.
