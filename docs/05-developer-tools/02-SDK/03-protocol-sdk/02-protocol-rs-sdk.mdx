...
<ol>
  <li>State changes are atomic - if a method fails, all changes are rolled back</li>
  <li>Events are only emitted if the transaction succeeds</li>
  <li>Read-only operations have no network overhead</li>
  <li>All public methods in the `#[app::logic]` block become available as application endpoints</li>
</ol>
