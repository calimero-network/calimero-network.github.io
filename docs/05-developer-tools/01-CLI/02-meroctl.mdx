...
- `identity` Create a context identity or it's alias
...
- `list` List identities in a context
  ...
  - Use `--owned` to get only owned identities
    ...
    ```bash
    meroctl --node-name <NAME> context identity list <contextId or alias> --owend
    ```
...
- `invite <context-alias> <inviter-alias> <invitee-alias>` Create invitation
  using aliases
- And more...

:::tip See the Invitations and Joinings guide for detailed examples of using
aliases to streamline context management. :::
