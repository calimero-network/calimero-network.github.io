...
Also, in the `impl` block of your main struct that holds the state, you need to have an 
`init` method denoted by the `#[app::logic]` attribute macro. That function should not take
any input, and it should return an instance of your struct. Think of it like a constructor for the main struct.
